...
if __name__ == '__main__':
    ...
    parser.add_argument('--warmup', default=0.2, type=float, help='Warmup epoch before using co-training')
    ...
    run_dir = 'training/tri_view/' + args.dataset + '/' + name_with_datetime(args.run_name)
    ...
    if args.loader == 'UCR':
        ...
        task_type = 'classification'
        train_data, train_labels, test_data, test_labels = datautils.load_tri_view(args.dataset)
    ...
    if args.irregular > 0:
        if task_type == 'classification':
            ...
            train_data[2] = data_dropout(train_data[1], args.irregular)
            test_data[2] = data_dropout(test_data[1], args.irregular)
        else:
    ...
    if args.eval:
        if task_type == 'classification':
            train_data, train_labels, test_data, test_labels = datautils.load_tri_view(args.dataset,args.data_perc)
