...
from datautils  import ThreeViewloader
class TS2Vec:
    '''The TS2Vec model'''
    ...
    def __init__(
        ...
        self._net_sea = TSEncoder(input_dims=input_dims, output_dims=output_dims, hidden_dims=hidden_dims, depth=depth).to(self.device)
        self.net_sea = torch.optim.swa_utils.AveragedModel(self._net_sea)
        self.net_sea.update_parameters(self._net_sea)
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False,logger=None):
        ...
        if self.max_train_length is not None:
            ...
            if sections >= 2:
                ...
                train_data[2] = np.concatenate(split_with_nan(train_data[2], sections, axis=1), axis=0)
        ...
        temporal_missing = np.isnan(train_data[2]).all(axis=-1).any(axis=0)
        if temporal_missing[0] or temporal_missing[-1]:
            train_data[2] = centerize_vary_length_series(train_data[2])
        ...
        train_data[2] = train_data[2][~np.isnan(train_data[2]).all(axis=2).all(axis=1)]
        
        train_dataset = ThreeViewloader(train_data)
        #train_dataset = TensorDataset(torch.from_numpy(train_data).to(torch.float))
        train_loader = DataLoader(train_dataset, batch_size=min(self.batch_size, len(train_dataset)), shuffle=True, drop_last=True)
        
        optimizer = torch.optim.AdamW(list(self._net.parameters())
                                      +list(self._net_freq.parameters())
                                      +list(self._net_sea.parameters()), lr=self.lr)
        ...
        while True:
            ...
            interrupted = False
            for batch,x,fft_x,seas_x in train_loader:
                ...
                seas_ts_l = seas_x.size(1)
                seas_crop_l = np.random.randint(low=2 ** (self.temporal_unit + 1), high=seas_ts_l+1)
                seas_crop_left = np.random.randint(seas_ts_l - seas_crop_l + 1)
                seas_crop_right = seas_crop_left + seas_crop_l
                seas_crop_eleft = np.random.randint(seas_crop_left + 1)
                seas_crop_eright = np.random.randint(low=seas_crop_right, high=seas_ts_l + 1)
                seas_crop_offset = np.random.randint(low=-seas_crop_eleft, high=seas_ts_l - seas_crop_eright + 1, size=seas_x.size(0))
                
                
                
                seas_out1 = self._net_sea(take_per_row(seas_x, seas_crop_offset + seas_crop_eleft, seas_crop_right - seas_crop_eleft))
                seas_out1 = seas_out1[:, -seas_crop_l:]
                
                seas_out2 = self._net_sea(take_per_row(seas_x, seas_crop_offset + seas_crop_left, seas_crop_eright - seas_crop_left))
                seas_out2 = seas_out2[:, :seas_crop_l]
                ...
                loss_fft = hierarchical_contrastive_loss(
                    out3,
                    out4,
                    temporal_unit=self.temporal_unit
                )
                loss_sea = hierarchical_contrastive_loss(
                    seas_out1,
                    seas_out2,
                    temporal_unit=self.temporal_unit
                )
                loss = loss + loss_fft  + loss_sea
                ...
                if self.n_epochs > (self.args.warmup):
                    ...
                    seas = torch.mean(seas_out1,dim=1) # out1.reshape(out1.shape[0],-1)
                    ...
                    loss_prototype_sea, cluster_result_sea['ma_centroids'] = prototype_loss_cotrain(seas, batch, cluster_result_sea, self.args)
    ...
    def encode(self, data, mask=None, encoding_window=None, causal=False, sliding_length=None, sliding_padding=0, batch_size=None):
        ...
        org_training_sea = self.net_sea.training
        self.net.eval()
        train_dataset = ThreeViewloader(data)
        #dataset = TensorDataset(torch.from_numpy(data).to(torch.float))
        loader = DataLoader(train_dataset, batch_size=batch_size)
        
        with torch.no_grad():
            output = []
            for batch,x,fft_x,seas_x in loader:
                #x = batch[0]
                ...
                out_seas = self._eval_with_pooling(seas_x, mask, encoding_window=encoding_window)
                #print("out shape:",out_fft.shape)
                # if encoding_window == 'full_series':
                #     out_fft = out_fft.squeeze(1)
                #print(out_fft.shape)
                out_seas= torch.mean(out_seas,dim=1)  #out_fft.reshape(out_fft.shape[0],-1)
            
                out = torch.cat((out, out_fft,out_seas), dim=1)  
        ...
        self.net_sea.train(org_training_sea)
        return output.numpy()
