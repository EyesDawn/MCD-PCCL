...
import numpy as np
from models import DualTSEncoder
...
class TS2Vec:
    ...
    def __init__(
        ...
        self._net = DualTSEncoder(input_dims=input_dims, output_dims=output_dims, hidden_dims=hidden_dims, depth=depth).to(self.device)
        self.net = torch.optim.swa_utils.AveragedModel(self._net)
        self.net.update_parameters(self._net)

        self._net_freq = DualTSEncoder(input_dims=input_dims, output_dims=output_dims, hidden_dims=hidden_dims, depth=depth).to(self.device)
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False,logger=None):
        ...
        while True:
            ...
            for batch,x,fft_x in train_loader:
                ...
                out1,temp_pred_l = self._net(take_per_row(x, crop_offset + crop_eleft, crop_right - crop_eleft))
                out1 = out1[:, -crop_l:]
                
                out2,temp_pred_r = self._net(take_per_row(x, crop_offset + crop_left, crop_eright - crop_left))
                out2 = out2[:, :crop_l]
                ...
                out3,freq_pred_l = self._net_freq(take_per_row(fft_x, fft_crop_offset + fft_crop_eleft, fft_crop_right - fft_crop_eleft))
                out3 = out3[:, -fft_crop_l:]
                
                out4,freq_pred_r = self._net_freq(take_per_row(fft_x, fft_crop_offset + fft_crop_left, fft_crop_eright - fft_crop_left))
                ...
                mse_loss_l = criterion(freq_pred_l,temp_pred_l.argmax(dim=1))
                mse_loss_r = criterion(freq_pred_r,temp_pred_r.argmax(dim=1))

                loss = (loss+loss2+mse_loss_l+mse_loss_r)
    ...
    def cls(self, train_data,train_labels, n_epochs=40, n_iters=None, verbose=False,logger=None):
        ''' Training the TS2Vec model.
        ...
        while True:
            ...
            for batch,x,_ in train_loader:
                ...
                y_true = torch.from_numpy(train_labels[batch])
                y_true = y_true.long()
                ...
                loss = criterion(temp_pred_l,y_true)
            ...
            loss_log.append(cum_loss)
            if logger:
                logger.debug(f"eval Epoch #{self.n_epochs}: loss={cum_loss}")
            self.n_epochs += 1
    ...
    def eval(self, train_data,train_labels, n_epochs=40, n_iters=None, verbose=False,logger=None):
        ''' Training the TS2Vec model.
        ...
        for batch,x,_ in train_loader:
            ...
            out1,temp_pred_l = self._net(take_per_row(x, crop_offset + crop_eleft, crop_right - crop_eleft))
            #out1 = out1[:, -crop_l:]
            total_acc.append(y_true.eq(temp_pred_l.detach().argmax(dim=1)).float().mean())
